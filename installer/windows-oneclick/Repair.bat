...
set "LAUNCHER_LOG=%LOG_DIR%\launcher-repair.log"
...
echo ================================================== > "%BAT_LOG%"
echo [%date% %time%] Iniciando Repair.bat >> "%BAT_LOG%"
echo [%date% %time%] Iniciando Repair.bat > "%LAUNCHER_LOG%"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_PS%" -Mode Repair -ProjectBundleUrl "%BUNDLE_URL%" >> "%BAT_LOG%" 2>&1
set "RUN_CODE=%errorlevel%"
if %RUN_CODE% neq 0 (
  echo [%date% %time%] Falha no Repair.bat. >> "%LAUNCHER_LOG%"
  echo [%date% %time%] Falha no Repair.bat. >> "%BAT_LOG%"
  start "" notepad "%LAUNCHER_LOG%"
  start "" notepad "%BAT_LOG%"
)
if %RUN_CODE% equ 0 (
  echo [%date% %time%] Repair concluido com sucesso. >> "%LAUNCHER_LOG%"
  echo [%date% %time%] Repair concluido com sucesso. >> "%BAT_LOG%"
)
echo.
